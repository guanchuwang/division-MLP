...
class FDMP_Module(nn.Module):
    ...
    def __init__(self, model, device):
        ...
        FDMP_Module.linear_cnt(self.model)
        FDMP_Module.convert_layers(self.model, self.device)
        # FDMP_Module.restore_layers(self.model) # restore the last linear layer
        print(self.model)
        # hegsns

    @staticmethod
    def linear_cnt(module):

        for name, child in module.named_children():
            if isinstance(child, nn.Linear):
                FDMP_Module.linear_num += 1

            else:
                FDMP_Module.linear_cnt(child)

    @staticmethod
    def convert_layers(module, device):
        for name, child in module.named_children():
            ...
            #
                ...
                # original_child = getattr(module, name)
                setattr(module, name, QReLU().to(device))
                # FDMP_Module.layer_num += 1

            elif isinstance(child, nn.Dropout):
                # original_child = getattr(module, name)
                setattr(module, name, QDropout(child.p).to(device))
                # FDMP_Module.layer_num += 1
            #
            ...
            #     FDMP_Module.layer_num += 1
                ...
                if True: # FDMP_Module.linear_counter < FDMP_Module.linear_num - 1:

                    original_child = getattr(module, name)
                    fdmp_layer = FDMP_Linear(child.in_features, child.out_features,
                                                      child.bias is not None).to(device)

                    setattr(fdmp_layer, "weight", original_child.weight)
                    setattr(fdmp_layer, "bias", original_child.bias)
                    setattr(module, name, fdmp_layer)

                FDMP_Module.linear_counter += 1

            else:
                FDMP_Module.convert_layers(child, device)
